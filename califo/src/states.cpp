...
const state states::READ_CONFIG = \
    {104, "Read config", actions::read_config };
const state states::INITIAL_CLEAR_MEMORY = \
    {105, "Initial clear memory", actions::initial_clear_memory };
const state states::APPLY_CONFIG = \
    {106, "Apply config", actions::apply_config };

// Normal states are 2xx
const state states::ACCUMULATE_PUBLISH_STATUS = \
    {201, "Accumulate publish status", actions::accumulate_publish_status };
const state states::ACCUMULATE_READ_SOCKET = \
    {202, "Accumulate read socket", actions::accumulate_read_socket };
const state states::NORMAL_PUBLISH_STATUS = \
    {203, "Normal publish status", actions::normal_publish_status };
const state states::NORMAL_READ_SOCKET = \
    {204, "Normal read socket", actions::normal_read_socket };
const state states::NORMAL_FIT_PEAK = \
    {205, "Normal fit peak", actions::normal_fit_peak };
