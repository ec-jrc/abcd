...
bool in_array(json_int_t channel, json_t *array)
{
    ...
    {
        for(size_t index = 0; index < json_array_size(array); index++)
        {
    ...
    while (terminate_flag == 0)
    {
        ...
        else if (size > 0 && result == EXIT_SUCCESS)
        {
            ...
            if (strstr(topic, "data_abcd_waveforms_v0") == topic)
            {
                ...
                json_t *samples = json_object();
                json_t *gates = json_object();
                ...
                while (input_offset < size)
                {
                    ...
                    if (!in_array(this_channel, active_channels))
                    {
                        ...
                        char number_buffer[sizeof("[16]")];
                        snprintf(number_buffer, sizeof("[16]"), "%d", this_channel);

                        json_object_set_new(samples, number_buffer, this_samples);
                        json_object_set_new(gates, number_buffer, these_gates);
                ...
                json_object_set_new(json_message, "module", json_string("wafi"));
                json_object_set_new(json_message, "timestamp", json_string(time_buffer));
                json_object_set_new(json_message, "msg_ID", json_integer(msg_ID));
                json_object_set_new(json_message, "active_channels", active_channels);
                json_object_set_new(json_message, "samples", samples);
                json_object_set_new(json_message, "gates", gates);

                char *output_buffer = json_dumps(json_message, JSON_COMPACT);
                if (output_buffer != NULL)
                {
                    ...
                    snprintf(new_topic, defaults_all_topic_buffer_size, "data_wafi_waveforms_v0_s%zu", output_size);

                    send_byte_message(output_socket, new_topic, output_buffer, output_size, verbosity);
                    msg_ID += 1;
                ...
                {
                    printf("ERROR: Unable to encode JSON object!!!\n");
                }
