...
then
    ...
    tmux new-window -d -c "${ABCD_FOLDER}" -P -t ABCD -n abad './abad2/abad2 -T 1 -D "tcp://*:16197" -f abad2/configs/AD2_SiPM.json'
    ...
    tmux new-window -d -c "${DATA_FOLDER}" -P -t ABCD -n dasa "${ABCD_FOLDER}/dasa/dasa -v -A tcp://127.0.0.1:16207"

    echo "Creating WaDi window"
    tmux new-window -d -c "${ABCD_FOLDER}" -P -t ABCD -n wadi './wadi/wadi -v'

    echo "Creating tofcalc windows"
    tmux new-window -d -c "${ABCD_FOLDER}" -P -t ABCD -n tofcalc "./tofcalc/tofcalc -f ./tofcalc/config.json"
