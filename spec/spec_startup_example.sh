...
# Checking if another spec session is running
...
tmux new-window -d -c "$FOLDER""/web_interface/" -P -t $MODULE -n gui 'node web_interface.js -v -m '"$MODULE"' -f '"$FOLDER"'/'"$MODULE"'/config.json -d '"$FOLDER"'/'"$MODULE"'/ -p 8081'

echo "Waiting for node.js to start"
sleep 2

echo "Creating ""$MODULE"" window"
tmux new-window -d -c "$FOLDER""/""$MODULE""/" -P -t $MODULE -n $MODULE "./""$MODULE"" -A 'tcp://127.0.0.1:16181'"

echo "System started!"
echo "Connect to GUI on address: http://127.0.0.1:8081"
