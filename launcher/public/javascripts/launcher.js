document.addEventListener('DOMContentLoaded', function () {
    ...
    const settings = {
        ...
        modules: {
            ...
            abcd: {
                label: 'abcd',
                unique_name: 'digitizer_interface',
                description: 'Digitizer interface',
            ...
            wadi: {
                label: 'wadi',
                unique_name: 'waveforms_display',
                description: 'Waveforms display',
            ...
            waan: {
                label: 'waan',
                unique_name: 'waveforms_analysis',
                description: 'Waveforms analysis',
            ...
            spec: {
                label: 'spec',
                unique_name: 'spectra_calculator',
                description: 'Spectra calculator',
            ...
            tofcalc: {
                label: 'tofcalc',
                unique_name: 'ToF_calculator',
                description: 'Time-of-Flight calculator',
            ...
            cofi: {
                label: 'cofi',
                unique_name: 'coincidences_filter',
                description: 'Coincidences filter',
    ...
    let working_directory = 'abcd_data/';
    ...
    group_modules.attr("abcd:modules:global_options:working_directory", working_directory);
    group_modules.attr("abcd:modules:global_options:experiment_name", experiment_name);
    ...
    function create_module_fieldset(id, module_id, executable, unique_name, description, options) {
        ...
        legend.textContent = unique_name;
        ...
        const label_unique_name = document.createElement("label");
        label_unique_name.textContent = "Unique name:";
        fieldset.appendChild(label_unique_name);

        const input_unique_name = document.createElement("input");
        input_unique_name.id = `${id}-input-unique_name`;
        input_unique_name.type = "text";
        input_unique_name.value = unique_name;
        input_unique_name.addEventListener("input", () => {
            console.log("Unique name changed:", input_unique_name.value)
            legend.textContent = input_unique_name.value;

            update_modules_attributes();
        });
        label_unique_name.appendChild(input_unique_name);

        fieldset.appendChild(document.createElement("br"));

        const label_description = document.createElement("label");
        label_description.textContent = "Description:";
        fieldset.appendChild(label_description);

        const input_description = document.createElement("input");
        input_description.id = `${id}-input-description`;
        input_description.type = "text";
        input_description.value = description;
        input_description.addEventListener("input", update_modules_attributes);
    ...
    function create_module(id, type, executable, unique_name, description, options, x, y) {
        ...
        module.attr("abcd:module:unique_name", unique_name);
        module.attr("abcd:module:description", description);
        ...
        create_module_fieldset(`abcd-fieldset-${id}`, id, executable, unique_name, description, options);
    ...
    function generate_startup_script(drawing) {
        ...
        const modules = drawing.find('.abcd-module').map((module) => ({
            ...
            unique_name: module.attr("abcd:module:unique_name"),
            description: module.attr("abcd:module:description"),
            ...
            socket_endpoints: module.find(".abcd-socket_endpoint").map((socket_endpoint) => ({
                ...
                address: `tcp://127.0.0.1:${port_numbers[socket_endpoint.attr("id")]}`
            }))
        }));

        const wit_configuration_file = `\${WORKING_DIRECTORY}/configs/wit_config_${experiment_name}.json`;

        const wit_configuration = {
            verbosity: 1,
            heartbeat: 1000,
            modules: modules.map((module) => ({
                name: module.unique_name,
                type: module.type,
                description: module.description,
                sockets: ((socket_endpoints) => {
                    let wit_sockets = [];

                    for (const socket_endpoint of socket_endpoints) {
                        if (socket_endpoint.direction === 'out' && socket_endpoint.type === 'status') {
                            // Status types are automatically attached to wit with separated streams: status and events
                            wit_sockets.push({
                                type: 'status',
                                address: socket_endpoint.address,
                                topic: `status_${module.type}`
                            });
                            wit_sockets.push({
                                type: 'events',
                                address: socket_endpoint.address,
                                topic: `events_${module.type}`
                            });
                        } else if (socket_endpoint.direction === 'out' && socket_endpoint.type.startsWith('data_')) {
                            // Only data types not generated by digitizers are attached
                            wit_sockets.push({
                                type: 'data',
                                address: socket_endpoint.address,
                                topic: `data_${module.type}`
                            });

                        } else if (socket_endpoint.direction === 'in' && socket_endpoint.type === 'commands') {
                            wit_sockets.push({
                                type: 'commands',
                                address: socket_endpoint.address
                            });
        ...
        script_content += `echo "Startup of experiment: ${experiment_name}"\n`;
        script_content += '\n';

        script_content += 'echo "Changing current directory to the user home: ${HOME}"\n';
        script_content += 'cd "${HOME}"\n';
        script_content += `WORKING_DIRECTORY="$(readlink -m "${working_directory}")"\n`;
        ...
        script_content += `echo "Creating wit configuration file: ${wit_configuration_file}"\n`;
        script_content += `cat <<-"EOF" > "${wit_configuration_file}"\n`;
        script_content += JSON.stringify(wit_configuration, null, 2);
        script_content += '\n';
        script_content += 'EOF\n';
        script_content += '\n';
        ...
        script_content += '\n';
        script_content += 'echo "Killing previous ABCD sessions"\n';
        script_content += 'tmux kill-session -t ABCD\n';
        ...
        script_content += `tmux new-window -d -c "/opt/local/abcd/wit/" -P -t ABCD -n wit "node app.js \\"${wit_configuration_file}\\""\n`;
        script_content += 'echo "Waiting for node.js to start"\n';
        script_content += 'sleep 2\n';
        script_content += '\n';

        modules.forEach((module, index) => {
            script_content += `echo "Creating the window for ${module.unique_name} of type ${module.type}"\n`;
    ...
    function sanitize_file_path(file_path) {
        return file_path.replace(/[^a-zA-Z0-9-_\.]/g, '').trim();
    }

    function update_global_options() {
        const working_directory_raw = document.getElementById('input-working_directory').value;
        working_directory = working_directory_raw.endsWith('/') ? working_directory_raw : working_directory_raw + '/';

        experiment_name = sanitize_file_path(document.getElementById('input-experiment_name').value);

        group_modules.attr("abcd:modules:global_options:working_directory", working_directory);
        group_modules.attr("abcd:modules:global_options:experiment_name", experiment_name);

        console.log(`Updated: working_directory: ${working_directory}; experiment_name: ${experiment_name}`);
    }
    ...
    function update_modules_attributes() {
        ...
        drawing.find('.abcd-module').forEach((module) => {
            ...
            const unique_name = fieldset.querySelectorAll('[id$="-input-unique_name"]')[0].value;
            const description = fieldset.querySelectorAll('[id$="-input-description"]')[0].value;
    ...
    document.getElementById('button-download_startup_script').addEventListener('click', function () {
        ...
        anchor.download = `ABCD_startup_${experiment_name}.sh`;
    ...
    document.getElementById('button-new_module').addEventListener('click', function () {
        ...
        create_module(`abcd-module_${counter_modules}`, new_module_type, new_module_type, settings.modules[new_module_type].unique_name, settings.modules[new_module_type].description, "", 0.2 * settings.modules.dimensions.width * settings.unit, 0.2 * settings.modules.dimensions.height * settings.unit);
        counter_modules += 1;
    });
    ...
    document.getElementById('input-working_directory').addEventListener('input', update_global_options);
    document.getElementById('input-experiment_name').addEventListener('input', update_global_options);

    // Creating the default set of modules
    create_module(`abcd-module_${counter_modules}`, 'abcd', 'abcd', settings.modules['abcd'].unique_name, settings.modules['abcd'].description, "-v -T 50 -f ./configs/abcd_config.json", 0.5 * settings.modules.dimensions.width * settings.unit, 2.0 * settings.modules.dimensions.height * settings.unit);
    counter_modules += 1;
    create_module(`abcd-module_${counter_modules}`, 'wadi', 'wadi', settings.modules['wadi'].unique_name + "_digitizer", settings.modules['wadi'].description, "", 2.0 * settings.modules.dimensions.width * settings.unit, 3.5 * settings.modules.dimensions.height * settings.unit);
    counter_modules += 1;
    create_module(`abcd-module_${counter_modules}`, 'waan', 'waan', settings.modules['waan'].unique_name, settings.modules['waan'].description, "-v -T 50 -f ./configs/waan_config.json", 2.0 * settings.modules.dimensions.width * settings.unit, 2.0 * settings.modules.dimensions.height * settings.unit);
    counter_modules += 1;
    create_module(`abcd-module_${counter_modules}`, 'wadi', 'wadi', settings.modules['wadi'].unique_name + "_waan", settings.modules['wadi'].description, "", 3.5 * settings.modules.dimensions.width * settings.unit, 0.5 * settings.modules.dimensions.height * settings.unit);
    counter_modules += 1;
    create_module(`abcd-module_${counter_modules}`, 'dasa', 'dasa', settings.modules['dasa'].unique_name, settings.modules['dasa'].description, "", 3.5 * settings.modules.dimensions.width * settings.unit, 2.0 * settings.modules.dimensions.height * settings.unit);
    counter_modules += 1;
    create_module(`abcd-module_${counter_modules}`, 'spec', 'spec', settings.modules['spec'].unique_name, settings.modules['spec'].description, "", 3.5 * settings.modules.dimensions.width * settings.unit, 3.5 * settings.modules.dimensions.height * settings.unit);
    counter_modules += 1;
    create_module(`abcd-module_${counter_modules}`, 'tofcalc', 'tofcalc', settings.modules['tofcalc'].unique_name, settings.modules['tofcalc'].description, "-f ./configs/tofcalc_config.json", 3.5 * settings.modules.dimensions.width * settings.unit, 5.0 * settings.modules.dimensions.height * settings.unit);
    counter_modules += 1;

    create_socket(SVG('#abcd-module_0-socket_1'), SVG('#abcd-module_1-socket_0'));
    create_socket(SVG('#abcd-module_0-socket_1'), SVG('#abcd-module_2-socket_1'));
    create_socket(SVG('#abcd-module_2-socket_2'), SVG('#abcd-module_3-socket_0'));
    create_socket(SVG('#abcd-module_2-socket_2'), SVG('#abcd-module_4-socket_1'));
    create_socket(SVG('#abcd-module_2-socket_2'), SVG('#abcd-module_5-socket_1'));
    create_socket(SVG('#abcd-module_2-socket_2'), SVG('#abcd-module_6-socket_1'));
