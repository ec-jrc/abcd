/*! \brief Determination of the trigger position of a periodic signal by
 *         employing a threshold relative to the pulse maximum.
 *
 ...
 *   Optional, default value: false
 * - `LT_analysis_start`: start of the interval in which the `trigger_position`
 *   will be searched
 * - `LT_analysis_end`: end of the interval in which the `trigger_position`
 *   should will be searched
 */
                        ...
                        void *user_config)
{
    ...
    if (offset_max < config->absolute_threshold) {
        ...
        if (!config->disable_LeftThr_gates) {
            ...
            uint8_t *additional_analysis = waveform_additional_get(waveform, index);
            index++;
            uint8_t *additional_baseline = waveform_additional_get(waveform, index);
            index++;
            uint8_t *additional_search = waveform_additional_get(waveform, index);
            index++;
