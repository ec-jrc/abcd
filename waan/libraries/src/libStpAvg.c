...
void energy_init(json_t *json_config, void **user_config)
{
    ...
    struct StpAvg_config *config = calloc(1, sizeof(struct StpAvg_config));
    ...
    read_config_number(json_config, rise_samples, 1, config);
                     ...
                     void *user_config)
{
    if (!user_config)
    {
        printf("ERROR: libStpAvg energy_analysis(): User config not defined, not performing analysis\n");
    ...
    const uint32_t bottomline_end = clamp(config->baseline_samples, 1, samples_number);
    const uint32_t topline_start = clamp(config->baseline_samples + config->rise_samples, 2, samples_number);
    const uint32_t topline_end = clamp(topline_start + config->baseline_samples, 3, samples_number);
    ...
    const double delta = (topline - bottomline);
    const double energy = delta * config->height_scaling;
    ...
    else
    {
        ...
        (*events_buffer)[0].qshort = clamp_to_uint16(risetime_samples);
        (*events_buffer)[0].qlong = clamp_to_uint16(delta);
