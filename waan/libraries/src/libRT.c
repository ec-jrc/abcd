...
void timestamp_init(json_t *json_config, void **user_config)
{
    ...
    struct RT_config *config = calloc(1, sizeof(struct RT_config));
    ...
    read_config_boolean(json_config, disable_RT_gates, false, config);
                        ...
                        void *user_config)
{
    if (!user_config)
    {
        printf("ERROR: libRT timestamp_analysis(): User config not defined, not performing analysis\n");
    ...
    {
        printf("ERROR: libRT timestamp_analysis(): Error status detected, not performing analysis\n");
