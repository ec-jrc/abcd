 ...
 */
void reallocate_curves(uint32_t samples_number, struct RT_config **user_config);
...
void timestamp_init(json_t *json_config, void **user_config)
{
    ...
    if (!json_is_object(json_config)) {
        printf("ERROR: libRT timestamp_init(): json_config is not a json_t object\n");

        (*user_config) = NULL;
    } else {
	struct RT_config *config = malloc(1 * sizeof(struct RT_config));

        if (!config) {
            printf("ERROR: libRT timestamp_init(): Unable to allocate config memory\n");
        ...
        if (json_is_boolean(json_object_get(json_config, "disable_RT_gates"))) {
            config->disable_RT_gates = json_is_true(json_object_get(json_config, "disable_RT_gates"));
        } else {
            config->disable_RT_gates = false;
        }
...
{
    struct RT_config *config = (struct RT_config*)user_config;
                        ...
                        void *user_config)
{
    ...
    if (config->is_error) {
        printf("ERROR: libRT timestamp_analysis(): Error status detected\n");
    ...
    unsigned int state = (config->curve_RT[0] < 0) ? STATE_BELOW_THRESHOLD : STATE_ABOVE_THRESHOLD;
    ...
    for (uint32_t index = 0; index < triggers_rising_counter; index++) {
        ...
        find_fine_zero_crossing(config->curve_RT, samples_number,
    ...
    if (!config->disable_RT_gates) {
        ...
        uint8_t *additional_RT_signal = waveform_additional_get(waveform, index);
        index++;
        uint8_t *additional_triggers = waveform_additional_get(waveform, index);

        const double RT_max = offset_max - threshold_value;
        const double RT_min = offset_min - threshold_value;
        const double RT_abs_max = (fabs(RT_max) > fabs(RT_min)) ? fabs(RT_max) : fabs(RT_min);

        const uint8_t ZERO = UINT8_MAX / 2;
        const uint8_t MAX = UINT8_MAX / 2;

        for (uint32_t i = 0; i < samples_number; i++) {
            additional_RT_signal[i] = (config->curve_RT[i] / RT_abs_max) * MAX + ZERO;
...
void reallocate_curves(uint32_t samples_number, struct RT_config **user_config)
{
    struct RT_config *config = (*user_config);
	...
	config->previous_samples_number = samples_number;
        ...
        double *new_curve_offset = realloc(config->curve_offset,
                                           samples_number * sizeof(double));
        double *new_curve_RT = realloc(config->curve_RT,
                                       samples_number * sizeof(double));
        ...
        if (!new_curve_samples) {
            printf("ERROR: libRT reallocate_curves(): Unable to allocate curve_samples memory\n");
        ...
        if (!new_curve_smoothed) {
            printf("ERROR: libRT reallocate_curves(): Unable to allocate curve_smoothed memory\n");
        ...
        if (!new_curve_offset) {
            printf("ERROR: libRT reallocate_curves(): Unable to allocate curve_offset memory\n");
        ...
        if (!new_curve_RT) {
            printf("ERROR: libRT reallocate_curves(): Unable to allocate curve_RT memory\n");

            config->is_error = true;
        } else {
            config->curve_RT = new_curve_RT;
        }
        if (!new_triggers_rising) {
            printf("ERROR: libRT reallocate_curves(): Unable to allocate triggers_rising memory\n");
        ...
        if (!new_triggers_falling) {
            printf("ERROR: libRT reallocate_curves(): Unable to allocate triggers_falling memory\n");
